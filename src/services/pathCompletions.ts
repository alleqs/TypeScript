...
namespace ts.Completions.PathCompletions {
    ...
    function getModulesForPathsPattern(fragment: string, baseUrl: string, pattern: string, fileExtensions: ReadonlyArray<string>, host: LanguageServiceHost): ReadonlyArray<NameAndKind> | undefined {
        ...
        const matches = tryReadDirectory(host, baseDirectory, fileExtensions, /*exclude*/ undefined, [includeGlob]).map(name => nameAndKind(name, ScriptElementKind.scriptElement, /*extension*/ undefined));
        const directories = tryGetDirectories(host, baseDirectory).map(d => combinePaths(baseDirectory, d)).map(directoryResult);

        // Trim away prefix and suffix
        return mapDefined(concatenate(matches, directories), ({ name, kind, extension }) => {
            const normalizedMatch = normalizePath(name);
            const inner = withoutStartAndEnd(normalizedMatch, completePrefix, normalizedSuffix);
            return inner !== undefined ? nameAndKind(removeLeadingDirectorySeparator(removeFileExtension(inner)), kind, extension) : undefined;
        });
    }
